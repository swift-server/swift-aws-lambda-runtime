...
extension _SpecialTreatmentEncoder {
    @inline(__always) fileprivate func wrapFloat<F: FloatingPoint & CustomStringConvertible>(_ float: F, for additionalKey: CodingKey?) throws -> YAMLValue {
    ...
    fileprivate func wrapEncodable<E: Encodable>(_ encodable: E, for additionalKey: CodingKey?) throws -> YAMLValue? {
        switch encodable {
        case let date as Date:
...
private struct YAMLKeyedEncodingContainer<K: CodingKey>: KeyedEncodingContainerProtocol, _SpecialTreatmentEncoder {
    typealias Key = K
    ...
    mutating func nestedUnkeyedContainer(forKey key: Self.Key) -> UnkeyedEncodingContainer {
        ...
        let nestedContainer = YAMLUnkeyedEncodingContainer(impl: impl, array: array, codingPath: newPath)
...
private struct YAMLSingleValueEncodingContainer: SingleValueEncodingContainer, _SpecialTreatmentEncoder {
    let impl: YAMLEncoderImpl
    let codingPath: [CodingKey]
...
extension EncodingError {
    ...
    fileprivate static func _invalidFloatingPointValue<T: FloatingPoint>(_ value: T, at codingPath: [CodingKey]) -> EncodingError {
        ...
        let debugDescription = "Unable to encode \(valueDescription) directly in YAML. Use YAMLEncoder.NonConformingFloatEncodingStrategy.convertToString to specify how the value should be encoded."
